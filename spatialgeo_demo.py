...
for thisLoop_guideline in loop_guidelines:
    ...
    while continueRoutine:
        ...
        if guidelines_img.status == STARTED:
            ...
            if tThisFlipGlobal > guidelines_img.tStartRefresh + 0.0-frameTolerance:
...
loop_videos = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('media/show_group_conditions.xlsx', selection=loop_videos_indices),
...
for thisLoop_video in loop_videos:
    ...
    for thisLoop_clip in loop_clips:
        ...
        loop_videos_indices = []
        for i in range(len(loop_videos.trialList)):
          loop_videos.trialList[i]['group_name']=='group_sunset'
          loop_videos_indices.append(i)
        # keep track of which components have finished
