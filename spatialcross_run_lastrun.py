...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Volumes/StateSpace/research_backup/Postdoc_Times/UTokyoPostdoc/CompNeuro/3 Projects/spatialnavigation/spatialdecisionmaking/neuroimage/psychopy/spatialcross/spatialcross_run_lastrun.py',
...
for thisLoop_video in loop_videos:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if show_single_clip.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > show_single_clip.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
