...
expName = 'spatial fast'  # from the Builder filename that created this script
expInfo = {'participant': '100', 'run': '00'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Volumes/bot/research_backup/Postdoc_Times/UTokyoPostdoc/CompNeuro/3 Projects/spatialnavigation/spatialdecisionmaking/neuroimage/psychopy/spatialcross/spatialcross_demo.py',
...
# Initialize components for Routine "instruction_r_"
instruction_r_Clock = core.Clock()
instruction_img_ = visual.ImageStim(
    win=win,
    name='instruction_img_', 
    image='sin', mask=None,
    ori=0.0, pos=(0, 0), size=(1.8, 1),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
instruction_key_ = keyboard.Keyboard()
...
show_single_video_rClock = core.Clock()
...
loop_instruction_r = data.TrialHandler(nReps=100.0, method='random', 
...
for thisLoop_instruction_r in loop_instruction_r:
    ...
    # ------Prepare to start Routine "instruction_r_"-------
    continueRoutine = True
    # update component parameters for each repeat
    instruction_img_.setImage('instruction/instruction_' + str(current_instruction) + '.png')
    instruction_key_.keys = []
    instruction_key_.rt = []
    _instruction_key__allKeys = []
    # keep track of which components have finished
    instruction_r_Components = [instruction_img_, instruction_key_]
    for thisComponent in instruction_r_Components:
    ...
    instruction_r_Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "instruction_r_"-------
    while continueRoutine:
        # get current time
        t = instruction_r_Clock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=instruction_r_Clock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        
        # *instruction_img_* updates
        if instruction_img_.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruction_img_.frameNStart = frameN  # exact frame index
            instruction_img_.tStart = t  # local t and not account for scr refresh
            instruction_img_.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruction_img_, 'tStartRefresh')  # time at next scr refresh
            instruction_img_.setAutoDraw(True)
        
        # *instruction_key_* updates
        waitOnFlip = False
        if instruction_key_.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruction_key_.frameNStart = frameN  # exact frame index
            instruction_key_.tStart = t  # local t and not account for scr refresh
            instruction_key_.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruction_key_, 'tStartRefresh')  # time at next scr refresh
            instruction_key_.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(instruction_key_.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(instruction_key_.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if instruction_key_.status == STARTED and not waitOnFlip:
            theseKeys = instruction_key_.getKeys(keyList=['left', 'right'], waitRelease=False)
            _instruction_key__allKeys.extend(theseKeys)
            if len(_instruction_key__allKeys):
                instruction_key_.keys = _instruction_key__allKeys[-1].name  # just the last key pressed
                instruction_key_.rt = _instruction_key__allKeys[-1].rt
    ...
    # -------Ending Routine "instruction_r_"-------
    for thisComponent in instruction_r_Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    loop_instruction_r.addData('instruction_img_.started', instruction_img_.tStartRefresh)
    loop_instruction_r.addData('instruction_img_.stopped', instruction_img_.tStopRefresh)
    # check responses
    if instruction_key_.keys in ['', [], None]:  # No response was made
        instruction_key_.keys = None
    loop_instruction_r.addData('instruction_key_.keys',instruction_key_.keys)
    if instruction_key_.keys != None:  # we had a response
        loop_instruction_r.addData('instruction_key_.rt', instruction_key_.rt)
    loop_instruction_r.addData('instruction_key_.started', instruction_key_.tStartRefresh)
    loop_instruction_r.addData('instruction_key_.stopped', instruction_key_.tStopRefresh)
    # the Routine "instruction_r_" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
while continueRoutine:
    ...
    if wait_for_start_key.status == STARTED and not waitOnFlip:
        theseKeys = wait_for_start_key.getKeys(keyList=['t', 'space'], waitRelease=False)
...
for thisLoop_video in loop_videos:
    ...
    show_single_clip = visual.MovieStim3(
        ...
        noAudio = False,
        filename="media/videos/"+str(town_weather_name)+".mp4",
    ...
    # keep track of which components have finished
    show_single_video_rComponents = [show_single_clip]
    for thisComponent in show_single_video_rComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    show_single_video_rClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "show_single_video_r"-------
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = show_single_video_rClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=show_single_video_rClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        ...
        if show_single_clip.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > show_single_clip.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in fix_cross_r_bqComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    loop_videos.addData('fix_cross_before_question.started', fix_cross_before_question.tStartRefresh)
    loop_videos.addData('fix_cross_before_question.stopped', fix_cross_before_question.tStopRefresh)
    ...
    for thisLoop_question in loop_questions:
        ...
        current_video_img=str(question_name)
        ...
        town_name_list = ["Town01","Town02","Town03","Town04","Town05","Town06","Town07","Town08"] 
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if key_question_for_know_or_not.status == STARTED and not waitOnFlip:
                theseKeys = key_question_for_know_or_not.getKeys(keyList=['left', 'right', '1', '2', '3', '4'], waitRelease=False)
            ...
            if right_arrow_image.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > right_arrow_image.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_question_for_know_or_not.status == STARTED:
                ...
                if (key_question_for_know_or_not.keys == 'left' or key_question_for_know_or_not.keys == '1' or key_question_for_know_or_not.keys == '3'):
                ...
                elif (key_question_for_know_or_not.keys == 'right' or key_question_for_know_or_not.keys == '2' or key_question_for_know_or_not.keys == '4'):
                    image_answer_green_square.pos=(0.5, 0.4)
                    image_answer_green_square.size=(0.22, 0.22)
                    image_answer_green_square.setImage("media/pic/green_square.png")
            ...
            if image_answer_green_square.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_answer_green_square.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        #        image_answer_for_know_or_not.setImage(correct_img_str)
            ...
            if (key_question_for_know_or_not.keys == str(correct_answer) or key_question_for_know_or_not.keys == str(correct_num_a) or key_question_for_know_or_not.keys == str(correct_num_b)):
                key_question_for_know_or_not.corr = 0
    ...
    for thisComponent in fix_cross_r_aqComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    loop_videos.addData('fix_cross_after_question.started', fix_cross_after_question.tStartRefresh)
    loop_videos.addData('fix_cross_after_question.stopped', fix_cross_after_question.tStopRefresh)
    thisExp.nextEntry()
